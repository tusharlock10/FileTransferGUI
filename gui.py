...
class ReceiverPage(Frame):
    def __init__(self, master):
        ...
        self.thread= None  # Stores the thread, so 
                           # that it can be killed later on, beautifully
        ...
        self.data_label_sender = Label(self, text='', font='"Product Sans" 12')
        self.data_label_msg = Label(self, text='', font='"Product Sans" 9')
        self.data_label_sender.pack()
        self.data_label_msg.pack()
    ...
    def check_messages_helper(self):
        while True:
            ...
            data = data.decode()
            self.data_label_sender['text'] = f'{addr[0]} says -'
            self.data_label_msg['text']=f'\n{data}'
